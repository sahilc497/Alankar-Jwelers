            ...
            <div className="lg:col-span-5 pt-8 md:pt-0">
              ...
              <h1 className="text-5xl md:text-7xl font-serif italic mb-8 leading-tight">
                {product.name.split(' ').map((word, i) => 
                  i === product.name.split(' ').length - 1 ? <i key={i} className="not-italic font-normal">*{word}*</i> : word + ' '
                )}
              </h1>
              ...
              <div className="mb-16">
                ...
                <ul className="space-y-4">
                  {product.details.map((detail, i) => (
                    <li key={i} className="flex justify-between text-sm font-sans tracking-wide border-b border-foreground/5 pb-2">
                       <span className="opacity-60">{detail.split(':')[0]}</span>
                       <span>{detail.split(':')[1] || detail}</span>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-8 border border-foreground/10 p-2">
                   <div className="flex items-center gap-4 px-4">
                      <button className="hover:text-accent transition-colors"><Minus size={14} /></button>
                      <span className="text-xs font-sans w-4 text-center">1</span>
                      <button className="hover:text-accent transition-colors"><Plus size={14} /></button>
                   </div>
      ...
      <section className="px-6 md:px-12 py-24 border-t border-foreground/10 mt-24">
        <div className="max-w-[1800px] mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
            {products.slice(0, 4).filter(p => p.id !== id).slice(0, 3).map(p => (
